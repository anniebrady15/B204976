---
title: "Does the prescription of antibiotics commonly used to treat respiratory infections correlate with COVID-19 cases in 2021?"
author: "Annie Brady"
date: '`r Sys.Date()`'
...
```
Then I downloaded the data sets for each month in 2021, so that I could plot a graph of the correlation between antibiotic prescriptions and COVID cases in 2021. I did this by creating a vector containing the file names. Then I used the `map()` function to apply `unction_clean_filter_drugs` to each month. I remaned the files in the 
```{r}
prescription_files <- c(
  "pitc202101.csv", "pitc202102.csv", "pitc202103.csv", "pitc202104.csv",
  "pitc202105.csv", "pitc202106.csv", "pitc202107.csv",
  "pitc202108.csv", "pitc202109.csv", "pitc202110.csv",
  "pitc202111.csv", "pitc202112.csv")
prescription_data_list <- map(prescription_files, function_clean_filter_drugs)
names(prescription_data_list) <- c(
  "January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"
)
```
Then I wrangled these to give totals for each health board. I chose to do total in each health board, firstly so I could easily join the data sets, but also so that later on I could compare the rates of antibiotic prescription in each health board, and see if there are any that are especially bad for over-prescription.
```{r}
function_summarise_prescriptions <- function(data, month_name) {
  data %>%
    group_by(hbt) %>%
    summarise(!!month_name := sum(paid_quantity)) %>%
    filter(hbt != "SB0806") #removing an error - this health board does not exist
}

summarised_prescriptions <- imap(prescription_data_list, ~ function_summarise_prescriptions(.x, .y))
```
Then I joined the totals from each month together, and wrangled the data using `pivot_longer` to create a dataset containing prescriptions per month for 2021.
```{r}
prescriptions_2021 <- reduce(summarised_prescriptions, full_join, by = "hbt")
...
prescriptions_monthly_totals %>%
  kable(col.names = c("Month", "Antibiotic Prescriptions"), align = "lc",
        caption = "Antibiotic Prescriptions per Month in Scotland, 2021") %>% 
  kable_styling("striped", full_width = FALSE)
```
### COVID Data

Next, I accessed COVID data from Public Health Scotland - this data set is "Weekly Tests and Cases by Health Board" from the "Viral Respiratory Diseases (Including Influenza and COVID-19) Data in Scotland" collection. I chose this set so that I could join it to my other data by health board.
```{r}
...
```
I then filtered for the year 2021, so that I could compare it to my prescriptions data from the same year. I wrangled the data so that it could be joined to `prescriptions_2021`. 
```{r}
...
prescriptions_covid_2021 <- prescriptions_monthly_totals %>%
  full_join(covid_monthly_total_2021, by = "month")
```
### Comparison
...
prescriptions_covid_2021 %>% 
  ...
  theme_minimal()
```
This graph shows a positive correlation between the number of antibiotic prescriptions and COVID cases. The grey area represents the 95% confidence interval for the fitted line.  
This correlation confirms the possibility that antibiotics may have been wrongly prescribed to treat COVID in 2021.   
A different theory to explain the correlation between antibiotic prescription and COVID cases is that some people may developed bacterial pneumonia after COVID, and would therefore be prescribed antibiotics. To investigate this, I accessed COVID hospital admissions data, titled 'Weekly Admissions by Age'. I used decided to use hosptial admissions data, as only severe COVID infections are admitted to hospital, and these cases are more likely to go on to develop bacterial pneumonia (Wu, Adhi & Highland, 2020). I filtered for the year 2021 and summarized to calculate total admissions per month. I then joined this to `prescriptions_monthly_totals` in order to visalise the trends in admissions and prescriptions on the same graph. I divided the prescriptions values by 1000 so that both variabled could be plotted on the same set of axis.
```{r}
...
```
This graph shows that there is not a clear correlation between the trends in Admissions and Prescriptions in 2021. However, in order to rule out COVID related bacterial pneumonia as the cause of increase antibiotic use, details on reasons for prescription, which is beyond the scope of this report.  
...
```
After I had joined the three data sets, and calculated Prescription per Capita, I wanted to visualise this on a map.  
To do this, I needed spatial data on the health boards. I joined this geometry data set to `prescriptions_hb_rates`.
...
rates_nhs_healthboards %>% 
  ...
  geom_sf_text(aes(label = hb_name), size = 2, color = "midnightblue", fontface = "bold")+ #label each health board on the map, specifying size, colour and font, to improve comprehension from the map.
...
```
This map shows that the Lanarkshire has considerably higher antibiotic prescriptions rates than other health boards in Scotland. More investigation is required to determine the cause of this. Some have suggested that the increase in telephone appointments with GPs due to lock-down restrictions limiting face-to-face appointments is to blame for the rise in antibiotic use (Gagliotti, *et al*. 2021). Further research could compare rates of telephone consultation in each health board to antibiotic prescription to ascertain whether this is true.
